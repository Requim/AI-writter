...
| 8 | `chapter_writer_node` | 章节内容填充 | 自动（3000-6000字） | `generate()` 场景填充 |
| 9 | `reflection_node` | 反思检查 | `interrupt()` 报告问题 | `structured_generate()` 7点检查 |
| 10 | `revision_node` | 修正节点 | `interrupt()` 4种决策 | `generate()` 修正 |
| 11 | `persist_node` | 持久化 | 自动（写DB + 存记忆） | 无 |
...
progress_check ────────────────────────────────→ (is_completed?)
   ├─ 是 → END
   └─ 否 → memory_retrieval_node → chapter_outline_node
                                    ↓
                              chapter_writer_node → reflection_node
                                    ├─ 通过 → persist_node → progress_check
                                    └─ 有问题 → revision_node → persist_node
```
...
**问题**：DeepSeek API 生成较长内容（章节细纲、总大纲等）耗时可能超过 180 秒，导致前端 axios timeout / 后端 `asyncio.wait_for` 超时断开。

**当前超时配置**（三层统一）：

| 层级 | 超时 | 文件 |
|------|------|------|
| LLM HTTP 客户端 | 180s | `infrastructure/llm/deepseek_adapter.py` |
| 应用层 asyncio | 180s | `api/routers/workflow_router.py` |
| 前端 axios | 180s | `writter_front/src/api/client.ts` |

**临时方案**：调大超时时间（治标不治本）。如仍超时可继续加大。

**长期方案**：见下方"首 Token 流式响应"。

### 首 Token 流式响应（待实现）

**问题**：`POST /v1/workflows/{thread_id}/invoke` 等待 LLM 完整响应后才返回，期间前端无任何反馈。

**方案**：使用已有的 `/stream` SSE 端点，前端在 LLM 首 token 到达后即可展示进度，不再受完整响应超时困扰。

#### 后端改动

1. `workflow_router.py` 的 `stream_workflow` 端点已实现 SSE 流式输出，直接可用
2. `NovelOrchestrator.stream()` 已实现，`astream()` 每步产出 chunk

#### 前端改动

```ts
// 原方案：POST 等待完整响应（可能超时）
const res = await workflowApi.invokeWorkflow(threadId, data)

// 新方案：EventSource 流式接收（首 token 即更新）
const eventSource = workflowApi.streamWorkflow(threadId)
...
**优点**：
- LLM 首 token 返回后即更新前端，无需等待完整生成
- 不再受完整响应超时限制（只要有 token 持续输出连接就不断）

**注意事项**：
- SSE 需要后端 `StreamingResponse` + 前端 `EventSource`
- interrupt 机制的响应也需要通过 SSE 流式传递（当前中断会阻塞流程）
