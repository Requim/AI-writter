...
class MemoryService(ABC):
    ...
    async def get_hierarchical_context(
        self, tenant_id: str, novel_id: str, current_index: int, m_count: int = 3
    ) -> str:
        """获取分层上下文：S层状态 + P层滚动规划 + M层近期章节 + L层摘要"""
        pass
    ...
    async def store_chapter_summary(
        self, tenant_id: str, novel_id: str, chapter_index: int, summary_content: str
    ) -> str:
