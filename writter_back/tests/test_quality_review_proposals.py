...
async def test_invalid_review_retries_once_then_offers_manual_fallback() -> None:
    ...
    review_state = {
        **state,
        **generated.update,
        "pending_proposal_decision": "accept",
    }
...
@pytest.mark.asyncio
async def test_revision_accept_uses_checkpointed_content_without_llm() -> None:
    ...
    assert command.update["current_chapter_content"] == "已保存的修订正文"
