...
class NovelOrchestrator(AgentOrchestrator):
    ...
    async def invoke(self, thread_id: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """启动新工作流"""
        print(f"{'='*60}", flush=True)
        print(f"【编排器·启动】进入 | thread_id={thread_id}, 输入字段={list(input_data.keys())}", flush=True)
        ...
        print(f"【编排器·启动】完成 | thread_id={thread_id}, 产生中断={'是' if has_interrupt else '否'}", flush=True)
        print(f"{'='*60}", flush=True)
        return result
    ...
    async def resume(self, thread_id: str, resume_value: Any) -> Dict[str, Any]:
        """恢复被中断的工作流"""
        print(f"{'='*60}", flush=True)
        print(f"【编排器·恢复】进入 | thread_id={thread_id}, 恢复值类型={type(resume_value).__name__}, 恢复值={str(resume_value)[:100]}", flush=True)
        await self._ensure_workflow()
        config = self._make_config(thread_id)
        # Use Command(resume=...) to properly resume from interrupt
        result = await self._workflow.ainvoke(
            Command(resume=resume_value),
            config,
        )
        has_interrupt = "__interrupt__" in str(type(result)) or (isinstance(result, dict) and "__interrupt__" in result)
        print(f"【编排器·恢复】完成 | thread_id={thread_id}, 产生中断={'是' if has_interrupt else '否'}", flush=True)
        print(f"{'='*60}", flush=True)
        return result
    ...
    async def stream(self, thread_id: str, input_data: Dict[str, Any]):
        ...
        print(f"{'='*60}", flush=True)
        print(f"【编排器·流式】开始 | thread_id={thread_id}, 输入字段={list(input_data.keys())}", flush=True)
        await self._ensure_workflow()
        config = self._make_config(thread_id)
        chunk_count = 0
        async for chunk in self._workflow.astream(input_data, config):
            chunk_count += 1
            print(f"【编排器·流式】数据块 #{chunk_count} | 类型={type(chunk).__name__}", flush=True)
            yield chunk
        print(f"【编排器·流式】结束 | thread_id={thread_id}, 总计数据块={chunk_count}", flush=True)
        print(f"{'='*60}", flush=True)
