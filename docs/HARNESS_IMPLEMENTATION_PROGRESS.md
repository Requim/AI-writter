...
- P6回滚点：前后端harness-51fdffc；发布目录releases/9d3fe4b，回滚层compose.rollback-p6.yml；保留0008与所有事实/运行记录，不执行数据库降级。须先暂停执行并确认活动租约为0。
...
### P6历史中间检查点（已由CP-014完成记录取代）

- 代码未提交、未部署，现网仍为P5。已实现固定13案例离线评估、姓名/前缀变形测试、强制人工审核与生成暂停、无正文运行指标、节点耗时采集隔离、CI服务预检与空库迁移门禁。
- 初步61项运行/评估/流程检查通过，13案例全部符合预期，Ruff和mypy57源文件通过。最终后端全量session12271在执行；前端测试36344、lint30767、build66367在执行，结束前必须读取结果。
- 隔离PostgreSQL/Redis预检通过，不可用Redis和非_test数据库均非零退出。Redis测试容器df9d949c及隧道47816已清理；仅保留P456测试PostgreSQL容器和19924隧道。
- 最新补充耗时采集异常不影响原结果，需要在最终全量后定向补跑test_harness_operations.py。运营说明见HARNESS_OPERATIONS.md。

### P5历史中间检查点（2026-09-07 21:57，已由CP-013取代）

- 工作区P5未提交、未部署；现网仍为P4。已新增0008_workflow_runtime、PostgresWorkflowCommandStore、ExecutionFence、FencedPostgresSaver、RuntimeJournal和GET事件重放接口，前端加入序号去重、缺口检测与GET恢复。
- 四项真实隔离PostgreSQL租约/接管/重放/孤儿任务测试通过（88.11秒）；流契约及静态23项通过；前端API7项通过、lint通过；新增代码继续验收中。后端全量测试session26767仍在执行。
- 因0008空库升级依赖，提前完成原P6的历史迁移冻结：0001/0002改用5ca52fa历史模型快照migration_baseline.py。独立空库到0008升级已通过。
- 待完成：检查点补偿退避/死信测试与人工重试入口、真实Postgres检查点fencing集成、业务写入入口覆盖审计、SSE恢复浏览器/权限测试、0007到0008与回滚演练、全量回归、阶段提交部署。P6评估集和运营开关尚未实施。
...
当前部署检查（2026-09-07 23:00）：P0至P6阶段代码已完成部署；当前前后端9d3fe4b、数据库0008，四个生产服务healthy，详见CP-014。
...
| P4 | 完成并部署 | 建立冲突中心、证据审阅和版本化纠错 | bba00e8，后端496项、前端124项通过 |
| P5 | 完成并部署 | 持久运行事件、SSE重放、共享租约和恢复机制 | 51fdffc，0008迁移与健康验证通过 |
| P6 | 完成并部署 | 建立评估集、CI门禁、可观测性和默认开启策略 | 9d3fe4b，后端517项、前端127项、13案例及线上保护检查通过 |
...
- [x] 新增“事实台账”和“冲突中心”接口
- [x] 新增原文证据展示、修订复验和版本化事实修正（显示证据原文，不伪造字符定位）
- [x] 新增“辛姓人物却出现陆氏祠堂”等回归样本

## P5-P6 运行与评估任务清单

- [x] 建立持久Run、Attempt、Checkpoint、Artifact、Event记录
- [x] 同小说执行互斥迁移到PostgreSQL lease并加入fencing
- [x] SSE单调序号、Last-Event-ID重放、客户端去重和gap检测
- [x] checkpoint reconciliation退避、死信、错误日志告警及人工重试接口
- [x] 服务启动识别过期orphan run，不误杀其他存活进程
- [x] 建立固定评估集：12个合成案例与1个用户反馈脱敏关系抽象案例；未纳入生产原稿
- [x] Provider异常契约、属性/变形、受控故障注入、真实数据库fencing及浏览器E2E
- [x] PostgreSQL/Redis CI fail-fast配置与本地等效正反向预检；GitHub远端尚未执行
- [x] 节点耗时、重试、事实审核、错误及持久运行终态指标
- [x] 默认事实保护、生成暂停与强制人工审核验证；生产默认开关已上线复核
...
- 2026-09-07 23:00（Asia/Shanghai）上线：代码9d3fe4bdbbc91e4517166e27c8bb27828c2d3d4e，提交信息feat(harness): 完成P6评估门禁与运营控制。前后端revision标签与提交一致；Git未推送，远端CI未执行。
- 后端镜像novel-writer-backend:harness-9d3fe4b，摘要sha256:34ad6e6ea9a3908ad34114123a4e3a1c7bcccda08b834a249514b6cb17a4afda；前端镜像novel-writer-frontend:harness-9d3fe4b，摘要sha256:26c15bf1465e4e17e6c43bf1e3b7a17e9058bd2322eff5da0bbc3480a5b54d07。
- 源码制品SHA256=bcd24cdbbff1eb1bd8499fa1b1f672ae34aec1ee81b4cc0784fcea18bbb1ad17；前端制品SHA256=57d1db19f5ca938c8814483fa14ede853d30ca78bdd05426b001795efee13e66；本地与服务器一致。没有新增依赖或迁移。
- 发布目录/opt/novel-writer/releases/9d3fe4b；compose.harness-p6.yml叠加此前全部发布层。应用回滚层compose.rollback-p6.yml指向前后端harness-51fdffc；不降级0008，不删除事实/事件/检查点，先停止当前执行并确认无活动租约。
- 四个生产服务healthy，后端/health/ready返回ready，前端回环5173首页200、未认证/api/v1/auth/me为401，nginx配置与reload通过。/api/health/ready并非已配置的代理路由，不用该路径判断后端健康。
- 生产数据库0008_workflow_runtime，小说7、章节15，活动租约0；数据库容器03ad289e2fd9与Redis容器8b687a8034f6及原数据卷均未改变。未写入业务数据、未调用Provider。
- 线上默认WORKFLOW_GENERATION_PAUSED=false、FACT_REVIEW_MODE=standard；容器内13案例、暂停保护、human_only保留硬门禁检查通过。策略覆盖仅在独立校验子进程内生效，未修改运行服务配置；暂停不会撤销已发出的Provider请求。
- 最后指标API定向验收1项通过，覆盖租户隔离、统计隐私与非法UUID；完整517项之后新增的采集故障保护由62项定向回归覆盖，不将其宣称为再次全量运行。
- 隔离测试、迁移恢复与Redis容器、相关SSH隧道、专用浏览器已清理。保留P5受限备份及原有本地开发服务器，无未结束验收进程。运营说明见docs/HARNESS_OPERATIONS.md。
- 后端完整517项通过，62项最终纯逻辑/工作流/静态定向通过，前端127项及lint/build通过，Ruff/mypy57源文件通过。
- 固定13案例全部符合预期，包括应阻断和应unknown的案例；8个姓名/前缀变形样本通过。没有调用真实Provider，不表示真实长篇准确率。
- 默认standard事实保护；human_only不沿用旧自动通过报告、不调用Judge，明确冲突仍阻断；暂停在领取生成命令前生效，保留阅读、人工编辑和取消。
- 节点计时异常不影响原业务结果，技术计时事件不进入作者进度面板；指标仅返回统计和状态，不返回正文、Prompt或令牌。
- 隔离服务预检通过；Redis不可用与非测试库名均以非零退出；CI先执行空库迁移再跑测试和评估，远端CI因未推送尚未运行。
- 最终浏览器375/768/1280px、事实两步确认、SSE断流重放全部通过；确认弹窗截图等待动画稳定后已目视核对。
- 仍未覆盖：真实供应商长篇生成、生产进程强杀演练、亲属/时间线/人物知识等新增事实类型的完整规则扩展。不能将本轮工程验收当作所有小说逻辑问题已消除。

### CP-013 P5发布前验收

- 2026-09-07 22:27已上线，代码51fdffc24df16630ca8e1bb0adfa5b383bbfaeb2；后端镜像sha256:be5662cbacd56157a3ffc031cfd5ef74f9f61ba00c454133a78e7fc7397872f0，前端sha256:92ceb09bbf1e22990a511305b28fa157ee72a24c90b1646062c2bd001d1017ec，revision标签一致。
- 四服务healthy，/health/ready=ready，首页200、未认证auth/me401；生产迁移0008，小说7、章节15，活动租约0；无Provider调用或业务写入。数据容器ID和卷均未变。
- 发布层releases/51fdffc/compose.harness-p5.yml；回滚层compose.rollback-p5.yml。源码制品SHA256=2a78cb9903d9774db908adff66381fa6413e8054d05b4a5756d42832af177a42，前端=f071eda64cc2a34e51c70eef196b5df8803b80b813cc689846177900fcaee23a，服务器校验一致。

- 完整后端500项通过（855.01秒），随后新增检查点/权限/恢复测试11项通过；实时钟及写入保护最终10项通过（130.32秒），恢复与流契约26项通过。Ruff、mypy52源文件通过。
- 前端126项、lint、build通过；浏览器实际fetch验证一次POST后断流，携带Last-Event-ID进行一次GET重放，重复事件被过滤，未触发模型。
- 新增0008运行表，Run每次Attempt独立留痕，事件先入库再推送；检查点产物记录引用已有LangGraph持久检查点。60秒租约每20秒续期，受工作流总截止时间约束；数据库实时钟及小说行锁校验fencing。
- 0001/0002已冻结到5ca52fa历史模型；空库到0008通过。生产备份恢复到独立容器，0007→0008→0007→0008通过，小说7、章节15数量不变；真实稿内容没有输出。
- 备份仅保留在服务器/opt/novel-writer/backups/p5-20260907-before-0008.dump，权限600，SHA256=1834f985e9987341fb0fc5125396cbb69afd18367b359dd47449f347d02d4810。恢复容器d905ce23及隧道38460已核对身份后删除，测试容器与19924隧道继续用于P6。
- 发布回滚采用前后端harness-bba00e8，不自动降级0008或删除事件。切换前必须确认无活动执行；旧进程全部停止后才切换租约实现。
- P5发布当时P6尚未实施；后续完成情况以CP-014为准。
...
- 事实审核新增“重新核对事实”，保留稿件、使用当前台账重新核对，硬冲突仍不可直接接受。
