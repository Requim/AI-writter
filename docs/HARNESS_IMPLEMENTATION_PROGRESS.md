...
| 阶段 | 状态 | 目标 | 主要验收证据 |
| --- | --- | --- | --- |
| P0 | 已验收待部署 | 建立恢复文档，清理基线失败，修复 fail-open，统一错误分类 | 后端 pytest、Ruff、mypy；前端 test、lint、build |
| P1 | 已验收待部署 | 重构“创作进度”、中文术语、用户状态与技术状态 | 组件测试、状态矩阵、三视口截图 |
