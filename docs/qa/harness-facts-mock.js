async (page) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    let data;
    if (path.endsWith('/facts')) data = { entities: [{ id: 'hero', entity_key: 'hero', name: '辛远', kind: 'character' }, { id: 'xin', entity_key: 'xin', name: '辛家', kind: 'family' }], fact_heads: [{ subject_id: 'hero', predicate: 'surname', value_text: '辛', version: 1, status: 'confirmed', valid_from_chapter: 1, evidence: { quote: '作者确认辛远姓辛', source_ref: 'synthetic' } }] };
    else if (path.endsWith('/facts/conflicts')) data = [{ chapter_id: 'chapter-1', chapter_number: 1, title: '旧宅来信', status: 'legacy_unverified' }];
    else if (path.includes('/facts/history/')) data = [{ subject_id: 'hero', predicate: 'surname', value_text: '辛', version: 1, status: 'confirmed', valid_from_chapter: 1, evidence: { quote: '作者确认辛远姓辛', source_ref: 'synthetic' } }];
    else if (path.endsWith('/auth/me')) data = { user, tenants: [tenant] };
