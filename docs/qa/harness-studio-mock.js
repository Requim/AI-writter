async (page) => {
  ...
  const proposal = { proposal_id: 'qa-review', kind: 'reflection', version: 1, chapter_number: 2, payload: { gate, issues: [] } };
  await page.route('**/api/v1/**', async (route) => {
    ...
    else if (path.endsWith('/state')) data = { thread_id: 'qa-novel', status: 'paused', interrupts: [{ action: 'quality_gate_human_review', chapter_number: 2, message: '计划兑现未确认或存在偏差，章节尚未归档，请核对本章要求后决定', proposal_id: proposal.proposal_id, proposal }], state: { current_chapter_index: 1, has_current_chapter_content: true }, next_nodes: ['reflection_review_node'] };
