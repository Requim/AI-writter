...
flowchart TD
    ...
    S --> O["生成/确认宏观总纲"]
    O --> F{"Schema 5 已生效？"}
    F -->|"是"| NP["生成并接受 NovelPlan 整书战略"]
    F -->|"否，Schema 2～4"| P["保存设定"]
    NP --> VP["装配分卷、剧情弧与全书章节槽位"]
    VP --> P
    P --> G["检查当前进度"]
    G --> M["检索前文记忆"]
    M -->|"Schema 5"| CQ["预占本章唯一额度"]
    CQ --> TW["按最新故事事实生成 3～7 章战术窗口"]
    TW --> CV["校验并装配槽位硬约束"]
    CV --> CO["生成当前章细纲与执行契约"]
    M -->|"Schema 2～4"| CO
    CO --> R{"Schema 5？"}
    R -->|"是"| CP["合并审核战术窗口与当前章细纲"]
    R -->|"否"| OR["审核当前章细纲"]
    CP --> W["按场景流式生成正文"]
    OR --> W
    W --> Q["质量审读"]
    Q -->|"通过"| PS["原子保存章节/进度/记忆/兑现结果"]
    Q -->|"未通过"| V["修订章节"]
    V --> Q
    PS --> G
```
...
1. 创作简报、角色设计与宏观总纲共同形成静态故事圣经。
2. Schema 5 先接受 `NovelPlan` 整书战略，再以 `VolumePlan/StoryArc` 约束分卷战役和全书章节槽位。
3. 每章根据最新故事事实生成不跨卷的 3～7 章 `TacticalWindow`，战术层只能安排推进方式，不得改写上层槽位硬约束。
4. 当前章细纲必须把每个必发事件、状态变化和伏笔任务映射到真实场景；战术窗口和细纲在人工模式下合并审核一次。
5. 正文按场景队列生成并通过 `content_delta` 实时推送，质量审读分别核对战略兑现和战术兑现。
6. 自动模式在上限内执行修订；手动模式通过 LangGraph interrupt 等待用户决策。章节完成后刷新下一窗口，用户停止时不提前调用模型。
7. 章节只有在完成审读后才进入数据库事务；失败 retry 从可信 checkpoint 的失败节点恢复，不重新生成已接受的上层结果。

权威顺序固定为：已完成正文与故事状态 > 结局/规模契约 > 分卷/核心剧情弧 > 锁定章节槽位 > 滚动战术 > 当前章自由发挥。Schema 5 在战术调用前只预占一次章节额度，战术、细纲和供应商重试不重复扣费；全局与租户开关会在工作流安全节点即时复核。Schema 2～4 仍沿用旧滚动规划兼容路径。
...
| P 层 | 版本化滚动战术 | 结合最新事实记录 3～7 章推进方式、压力升级和出口钩子 |
...
| `NOVEL_PLANNING_V1_ENABLED` | `false` | 整书规划全局 kill switch；还需平台管理员为具体租户开启后才生效 |
